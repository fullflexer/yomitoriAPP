...
async function renderFixture(baseName: string, fixture: GoldFixture) {
...
function buildLayout(baseName: string, fixture: GoldFixture) {
  ...
  fixture.persons.forEach((person, index) => {
    ...
    rowValues.forEach((value, valueIndex) => {
      ...
      commands.push(
        ...wrapText(value, columns[valueIndex].width - 28, fontSize).map((line, lineIndex) => ({
          type: "text" as const,
          x: valueX + 14,
...
function summarizeEvents(person: GoldPersonField) {
...
function drawWithCanvas(context: CanvasRenderingContext2DLike, commands: DrawCommand[]) {
...
function renderSvg(width: number, height: number, commands: DrawCommand[]) {
  const body = commands
    .map((command) => {
      if (command.type === "rect") {
...
async function loadSharpRenderer(): Promise<SharpRenderer> {
  ...
  const sharpModule = (await import(pathToFileURL(sharpModulePath).href)) as {
    default?: SharpRenderer;
  };
